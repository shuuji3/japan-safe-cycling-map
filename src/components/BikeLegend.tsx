...
export function BikeLegend({ visible, onToggle, open, handleOpen, showFooter = true }: Props) {
  ...
  const placeNote = (e: React.MouseEvent<HTMLButtonElement>, id: string): void => {
    ...
    <>
      <div className="bike-sidebar">
        <div className="legend-head" onClick={handleOpen}>
          ...
          <span className="legend-fold" title={open ? t`閉じる` : t`開く`}>
        ...
        <div className={open ? 'legend-collapse open' : 'legend-collapse'}>
          <div className="legend-collapse-inner">
            ...
            {showFooter && <SourceFooter locale={locale} onSwitch={(l) => i18n.activate(l)} />}
          </div>
        </div>
      </div>
      {openDef && (
          ...
          {openDef.attrs.map((a, i) => (
            <div key={i} className="note-attr">
              <div className="note-attr-tags">
                {a.tags.map((t) => (
                  <code key={t} className="bike-toggle-tag">
                    {t}
                  </code>
                ))}
              </div>
